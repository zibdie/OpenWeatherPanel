...
checkCondition = (description) => {
    ...
    : description.includes('storm') ? 'storm'
    : description.includes('snow') ? 'snow'
    : description.includes('mist') ? 'mist'
    : null;
}
...
methodResponse = (input, city_null_check) => {
    ...
    :  (city_null_check == "" && input == "geo") ? "Weather at the moment based on your browsers GPS coordinates "
    : input == "ip" ? "According to your Internet Provider, it seems you are located in "
    : input == "zip" ? "The Zip You Enterered Is "
    : null;
}
...
function prepareData(weather_response, method)
{
   ...
   infoData = { 
	...
	"weather_response": {
       ...
       "current_weather_descp": UpperCaseSentence(weather_response['weather'][0]['description']),
    ...
    let cssAdd =  jsonFile[timeofdayint(time)][checkCondition(weather_response['weather'][0]['description'])]
    infoData['css'] = cssAdd;
    infoData['api'] = {"time" : timeofdayint(time), "conditions": checkCondition(weather_response['weather'][0]['description'])}
...
app.get("/api/getWeather", cors(), (req, res) => 
{
        ...
        {
            let methodSend = "ip";
            if(!(req.query['ip']).match('^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$'))
            {
                res.send({"status": 0, "message": "Invalid IP Input (Only IPv4 IP addresses are supported)"});
            }
            else
            {
                axios.get(`http://ip-api.com/json/${req.query['ip']}`)
                .then(resp => resp.data)
                .catch(error => console.error("IP Fetch Failed: " + error))
                .then(data => axios.get(`https://api.openweathermap.org/data/2.5/weather?zip=${data['zip']},${data['countryCode']}&units=imperial&appid=${openweathermap_apikey}`))
                .catch(error => {
                    //If we run into trouble getting the users location, just fetch New York City as a default
                    methodSend = "for";
                    return axios.get(`https://api.openweathermap.org/data/2.5/weather?zip=10010,us&units=imperial&appid=${openweathermap_apikey}`)
        ...
        else if(req.query['geoip'])
        {
            ...
            {
                let geosplit = (req.query['geoip']).split(",")
                axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${geosplit[0]}&lon=${geosplit[1]}&units=imperial&appid=${openweathermap_apikey}`)
                .then(resp => resp.data)
